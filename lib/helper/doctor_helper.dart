...
import 'package:aiataf/models/doctor_list_model.dart';
...
class DoctorHelper {
  ...
  Future<Database> connectToDatabase() async {
    ...
    String path = '$directory/doctorlist.db';
    return openDatabase(
      path,
      version: 1,
      onCreate: (db, version) {
  ...
  Future<List<DoctorListData>> getAllRecipes() async {
    List<Map<String, dynamic>> tasks = await database.query(tableName);

    print("brand data---------------------.${tasks}");

    print("grand length-------------------->${tasks.length}");
    return tasks.map((e) => DoctorListData.fromJson(e)).toList();
  }

  insertNewRecipe(DoctorListData recipeModel) {
